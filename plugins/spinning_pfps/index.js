...
const { pfpsSlashCmds } = require('./lib/pfps');

const pfpsPluginSlashCmds = [...pfpsSlashCmds];

module.exports = {
  api: {
    slashCommands: pfpsPluginSlashCmds
  },

  async init(config, runtime) {
    const coreApi = runtime.getPluginAPI?.('spinning_core');
    if (coreApi?.registerSlashCommand) {
      for (const cmd of pfpsPluginSlashCmds) {
  ...
  hooks: {
    interaction_received: async (payload, runtime) => {
      ...
      const cmd = pfpsPluginSlashCmds.find(c => c.name === interaction.commandName);
      if (!cmd) return;

      try {
        await cmd.execute(interaction, runtime);
      } catch (e) {
        console.error(`[spinning_pfps] Error /${interaction.commandName}:`, e.message);
