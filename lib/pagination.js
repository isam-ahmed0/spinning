...
function createPaginationSession(options) {
  ...
  if (!interactionOrMessage) throw new Error('interactionOrMessage is required');
  if (!pages) throw new Error('pages is required');
  if (!renderPage || typeof renderPage !== 'function') throw new Error('renderPage must be a function');
  ...
  async function getPageData(pageIndex) {
    if (Array.isArray(pages)) return pages[pageIndex];
    if (typeof pages === 'function') return await pages(pageIndex);
  ...
  function createPaginationButtons(currentPage, totalPages) {
    ...
    actionRow.addComponents(
      new ButtonBuilder()
        ...
        .setLabel(`${currentPage + 1}/${state.totalPages}`)
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(true)
    );
    actionRow.addComponents(
    ...
    state.collector.on('collect', async (i) => {
      try {
        ...
        switch (action) {
          ...
          case 'prev': newPage = Math.max(0, state.currentPage - 1); break;
          case 'next': newPage = Math.min(state.totalPages - 1, state.currentPage + 1); break;
  ...
  return { renderInitial, attachCollector, stop, getState };
}

module.exports = { createPaginationSession };
